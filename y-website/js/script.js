$(document).ready(() => {
	...
	$(".step-item.img").hide();
	$(".step-item.img.active").show();

	$(".step-list-item").on("click", function (evt) {
		evt.preventDefault();
		...
		if (!$(this).hasClass("active")) {
			...
			var sel = this.getAttribute("data-tab");
			$(".step-item.img").hide();
			$(".step-item.img").filter(sel).fadeIn(500, "linear");
		}
	});
	...
	if ($(".country__list")) {
		...
		$.getJSON("https://restcountries.eu/rest/v2/all", function (data) {
			...
			$(".btn-country-list").click(function () {
				$("html, body").animate(
					{
						scrollTop: $("#list-az").offset().top - 100,
					},
					500
				);
	...
	$(".chat__icon").click(function (e) {
		e.preventDefault();
		$(".chat__window").toggleClass("open");
	});

	$(".btn-item.close").click(function (e) {
		e.preventDefault();
		$(".chat__window").toggleClass("open");
	});

	// title
	let $sectionMain = $("section.talk-more");
	if ($sectionMain.length) {
		let $talkSliderItem = $sectionMain.find(".talk-more__slider > .letter");
		let u = 0;
		setInterval(function() {
			$talkSliderItem.eq(u % $talkSliderItem.length).removeClass('is-visible').addClass('is-hidden');
			u++;
			$talkSliderItem.eq(u % $talkSliderItem.length).removeClass('is-hidden').addClass('is-visible');
		}, 2500);
	}
	...
	if ($(".choose-country__search-input")) {
		$.getJSON("https://restcountries.eu/rest/v2/all", function (data) {
			...
			$countrySearch.keyup(function () {
				...
				if (count === 0 && $value != 0) {
					searchedCountry =
						'<li class="text-md text-white md:text-base">Sorry! No match!</li>';
				}
				//console.log(count);
